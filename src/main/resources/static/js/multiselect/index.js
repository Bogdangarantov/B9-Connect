...
var drop = function(info){var o = {
   ...
   open: false,
   html: {
      select: tt.get(info.selector)[0],
      options: tt.get(info.selector + ' option'),
      parent: undefined,
   },
   init: function(){
      this.html.parent = tt.get(info.selector)[0].parentNode
      this.html.drop = tt.template('<div class="drop"></div>')
      this.html.dropDisplay = tt.template('<div class="drop-display">Display</div>')
      this.html.dropOptions = tt.template('<div class="drop-options">Options</div>')
      this.html.dropScreen = tt.template('<div class="drop-screen"></div>')
   ...
   renderDrop: function(){ 
      var that = this;
      var parentHTML = tt.template('<div></div>')
      this.selected.forEach(function(select, index){ 
         var option = that.options[select.index];
         var childHTML = tt.template('<span class="item '+ select.state +'">'+ option.html +'</span>')
         var childCloseHTML = tt.template(
